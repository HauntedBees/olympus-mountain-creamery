...
func get_actions_as_json() -> String:
	return JSON.stringify(get_actions_as_dictionary())
	...
	for action_name in actions.keys():
		...
		match split_type[0]:
			...
			"InputEventMouseButton":
				...
				event.button_pressed = true
				event.button_index = int(split_type[1]) as MouseButton
				InputMap.action_add_event(action_name, event)
			"InputEventJoypadButton":
				var event := InputEventJoypadButton.new()
				event.button_pressed = true
				event.button_index = int(split_type[1]) as JoyButton
				InputMap.action_add_event(action_name, event)
			"InputEventJoypadMotion":
				var event := InputEventJoypadMotion.new()
				event.button_pressed = true
				event.axis = int(split_type[1]) as JoyAxis
